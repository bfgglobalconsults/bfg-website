...
const ClockInForm = ({ employees, selectedEmployee, onSuccess, onCancel }) => {
  const [formData, setFormData] = useState({
    ...
    work_mode: 'Office',
    reason: '',
    clock_in_time: new Date().toISOString().slice(0, 16) // YYYY-MM-DDTHH:MM format
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        clock_in_time: new Date(formData.clock_in_time).toISOString()
      };

      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/attendance/clock-in`, payload);
      ...
      if (error.response?.status === 403) {
        ...
        toast.error('Failed to clock in employee. Please try again.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {selectedEmployeeData ? (
          <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium text-green-800">{selectedEmployeeData.fullName}</p>
                <p className="text-sm text-green-600">{selectedEmployeeData.department} • {selectedEmployeeData.jobTitle}</p>
              </div>
              <button
                type="button"
                onClick={() => {
      ...
      <div>
        ...
        <textarea
          ...
          onChange={handleInputChange}
          placeholder="Enter reason for clock-in (e.g., working from home due to weather conditions)"
          rows={3}
          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
      </div>
