...
const NotifyModal = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // await fetch('/api/notify', { method: 'POST', body: JSON.stringify(form) })
    setTimeout(() => {
      setSubmitting(false);
      onSubmit();
      onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl p-8 w-full max-w-md shadow-lg relative max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-gray-500 mb-6 text-center">Be the first to know when Appetiza.ai launches!</p>
...
const ComingSoonCard = ({ onNotify }) => (
  <div className="bg-gradient-to-br from-orange-100 to-orange-50 border-2 border-orange-400 rounded-2xl p-8 flex flex-col items-center shadow-lg">
    <span className="uppercase tracking-widest text-orange-700 font-bold mb-2">Coming Soon</span>
    <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 text-center mb-4">Be the First to Access Appetiza.ai</h2>
    <p className="text-lg text-gray-700 text-center mb-6 max-w-xl">
      <span className="font-semibold">Business Planning Made Seamless</span><br/>
      A powerful new SaaS platform is coming soon... <span className="font-semibold text-orange-700">Appetiza.ai</span> is built to make business planning easy, intelligent, and fully guided. Whether you&apos;re launching a startup or refining your strategy, Appetiza.ai simplifies the process every step of the way.
    </p>
    <button className="bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-8 rounded-lg text-lg shadow transition" onClick={onNotify}>Notify Me</button>
  </div>
);

const FeatureCard = ({ icon, title, points, image, reverse }) => (
  <div className={`flex flex-col-reverse md:flex-row ${reverse ? 'md:flex-row-reverse' : ''} items-center gap-8 my-12`}>
    <div className="md:w-1/2 flex flex-col gap-4">
      <span className="inline-block px-4 py-2 bg-white border-2 border-orange-500 rounded-2xl text-orange-700 font-semibold w-fit mb-2">{icon}</span>
      <h3 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{title}</h3>
      <ul className="space-y-3">
        {points.map((pt, i) => (
          <li key={i} className="flex items-start gap-3">
            <span className="flex items-center justify-center rounded-full bg-orange-500 text-white font-bold" style={{ width: 32, height: 32, minWidth: 32, minHeight: 32 }}>{i + 1}</span>
...
const SelectedProductCards = () => (
  <div className="my-16">
    <h3 className="text-2xl font-bold text-center mb-6 text-gray-800">Selected Product Highlights</h3>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow flex flex-col items-center">
        <Image src={ProductImage} alt="Business Planning" className="w-32 h-32 object-contain mb-4" />
        <h4 className="font-bold text-lg mb-2">AI-Driven Business Planning</h4>
        <p className="text-gray-600 text-center">Automate and optimize your business plan with intelligent recommendations and step-by-step guidance.</p>
      </div>
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow flex flex-col items-center">
        <Image src={ProductExpand} alt="Expansion" className="w-32 h-32 object-contain mb-4" />
        <h4 className="font-bold text-lg mb-2">Global Expansion Toolkit</h4>
        <p className="text-gray-600 text-center">Expand confidently with built-in tools for market research, compliance, and localization.</p>
      </div>
    </div>
  </div>
);
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl p-8 shadow-lg text-center max-w-sm mx-auto">
            ...
            <p className="text-gray-700 mb-4">You&apos;ll be the first to know when Appetiza.ai launches.</p>
            <button className="mt-2 px-6 py-2 bg-orange-600 text-white rounded-lg font-semibold" onClick={() => setShowSuccess(false)}>Close</button>
          </div>
        </div>
      )}
    </div>
